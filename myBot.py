...
# 			await client.get_channel(762916201654386701).send(e)
	...
	##########
	RomeNumbers = ('L', 'XLIX', 'XLVIII', 'XLVII', 'XLVI', 'XLV', 'XLIV', 'XLIII', 'XLII', 'XLI', 'XL', 'XXXIX', 'XXXVIII', 'XXXVII', 'XXXVI', 'XXXV', 'XXXIV', 'XXXIII', 'XXXII', 'XXXI', 'XXX', 'XXIX', 'XXVIII', 'XXVII', 'XXVI', 'XXV', 'XXIV', 'XXIII', 'XXII', 'XXI', 'XX', 'XIX', 'XVIII', 'XVII', 'XVI', 'XV', 'XIV', 'XIII', 'XII', 'XI', 'X', 'IX', 'VIII', 'VII', 'VI', 'V', 'IV', 'III', 'II', 'I')
	
	Tears = ('아톰', '몰레큘', '셀', '슈퍼셀', '하이퍼셀', '워터', '클린워터', '아이스', '클린아이스', '하드아이스', '소일', '우드', '페이퍼', '샌드', '글래스', '클리어글래스', '미러', '클리어미러', '스톤', '하드스톤', '아이언', '하드아이언', '브론즈', '클리어브론즈', '브론즈메달', '실버', '클리어실버', '실버메달', '골드', '클리어골드', '골드메달', '아메티스트(자수정)', '크리스탈', '클리어크리스탈', '블랙크리스탈', '다이아몬드', '블랙다이아몬드', '아다만티움', '사파이어', '에메랄드', '루비', '우루', '비브라늄', '클래식', '프리미엄', '딜럭스', '익스트림', '플래티넘', '미스틱', '챌린저', '마스터', '그랜드', '챔피언', '레전드')
	...
	if message.channel.name == "랭크업" or message.channel.name == "시간의-방": # 랭크업 가능한 채널의 메시지일경우
		...
		if rankup in commands:
			...
			if ifRankup: # 랭크업 했을경우
				...
				await message.author.remove_roles(role) # 현재 랭크 제거
				
				if RomeNumbers.index(RomeNumber) == len(RomeNumbers)-1: # 마지막 랭크일경우
					try:
						role = discord.utils.get(message.guild.roles, name=RomeNumbers[0]) # 처음 랭크로 돌아가고
